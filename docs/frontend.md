...
| 📅 **每日挑战** | 10 轮 | 不限时 | 每天一组全球固定 10 题，注册用户每天一次，全球同题竞技（v1.16.0 新增） |
| ⚔️ **对战模式** | 5 轮 | 每轮 60s | 实时匹配 1v1，比拼总分（v1.16.0 新增，需后端在线） |
...
- 🔗 **成绩在线化** — 游客自动接入后端，历史记录与最佳成绩优先走服务端，离线自动降级（v1.15.0）
- 👤 **账号系统** — 登录 / 注册 / 游客进度绑定（v1.15.0）
- 📊 **个人统计** — 账号面板展示累计局数/轮数/总分/最佳/命中率/分模式统计（v1.16.0）
- 🏆 **排行榜** — 各模式总榜与今日榜，取最高分排名，当前玩家高亮（v1.16.0）
- 📅 **每日挑战** — 每日固定 10 题全球同题，注册用户每天一次（v1.16.0）
- ⚔️ **多人对战** — socket.io 实时匹配 1v1，5 题 60 秒限时 PK（v1.16.0）
- 🏠 **对战私房** — 创建 6 位房间码私房，好友凭码直连开局（v1.19.0）
- 🤝 **好友系统** — 按用户名添加好友，请求/接受/删除，账号面板实时展示（v1.19.0）
- 🎯 **天梯排位** — 对局自动结算评分，青铜→宗师 7 段位 + 赛季榜 + 连胜记录（v1.19.0）
- 🗺️ **地点图鉴** — 答对即点亮地点，统计作答次数与首次/最近点亮时间（v1.19.0）
- 🕹️ **对局回放** — 历史对局逐轮地图复盘，猜测与答案坐标连线展示（v1.19.0）
- 💡 **地点冷知识** — 结算弹窗自动展示该地点的趣味知识（v1.19.0）
- 🔑 **第三方登录** — 支持 Google 账号一键登录（服务端配置后生效）（v1.19.0）
- 📦 **图包工坊** — 注册用户地图选点创建自定义题库，公开分享全球玩家，成绩隔离不计入排行榜/天梯（v2.0.0）
...
| `src/js/config.js`                     | 全局配置：`VERSION` / `CHANGELOG` / 模式与分区计分参数（无任何密钥）            |
| `src/js/data.js`                       | 地点题库 `LOCATIONS`（1570 条）+ 派生题库 `WORLD_LOCATIONS` / `CHINA_LOCATIONS` |
| `src/js/game.js`                       | 游戏逻辑：状态、抽题、计分、街景加载、UI 交互                                   |
| `src/js/api.js`                        | 后端 API 客户端：令牌管理、401 自动刷新、离线降级                                |
| `src/js/auth.js`                       | 账号面板：登录 / 注册 / 游客绑定 / 个人统计展示                                   |
| `src/js/lb.js`                         | 排行榜面板：总榜/今日榜切换、模式切换、当前玩家高亮（v1.16.0）                   |
| `src/js/daily.js`                      | 每日挑战面板：拉取今日题单、开始挑战（v1.16.0）                                  |
| `src/js/mp.js`                         | 多人对战：socket.io 匹配/回合/结算客户端（v1.16.0）                               |
| `src/js/packs.js`                      | 图包工坊：图包列表/创建/地图选点/编辑器（v2.0.0）                                  |
...
| 版本        | 日期       | 亮点                                                                                          |
| ----------- | ---------- | --------------------------------------------------------------------------------------------- |
| **v2.0.0**  | 2026-08-18 | 📦 图包工坊 + 🚀 排行榜性能重构 + 🔒 全模块安全审计加固 + 🛠 生产部署双形态（Compose 1C1G / systemd 单二进制） |
| **v1.19.0** | 2026-08-17 | 🤝 好友 + 🎯 天梯排位 + 🏠 对战私房 + 🗺️ 地点图鉴 + 🕹️ 对局回放 + 🏅 每日计分榜 + 💡 冷知识 + ❤️ 赞助名单 + 🔑 第三方登录（Google）+ 🚀 CDN 直连带宽优化 |
| **v1.18.0** | 2026-08-07 | 🏆 成就与称号 + 🗼 地标模式 + 📱 PWA（可安装/离线壳）+ 📊 后端指标与数据源抽象                |
| **v1.17.0** | 2026-08-06 | 🔐 街景查看器迁移（移除前端密钥，360° 查看器走代理）+ 🏆 成绩服务端防伪校验                  |
| **v1.16.0** | 2026-08-06 | 📅 每日挑战 + 🏆 排行榜 + 📊 账号统计升级 + ⚔️ 多人对战（测试）                             |
| **v1.15.0** | 2026-08-06 | 🔗 成绩在线化 + 👤 账号系统 + 📡 街景搜索服务端代理                                        |
