...
const CHANGELOG = [
    {
        ...
        changes: [
            '🗺️ 地图开关重做：地图内新增 ✕ 关闭按钮，并把「🗺️ 地图」按钮扩展到所有设备 —— 此前移动端存在地图展开后关不掉的问题（外部按钮被左下角计时器压住，且层级低于展开的地图）。',
            '🔧 修复移动端地图收不起来：开关按钮 z-index 由 1002 提升到 2000（高于展开态地图的 1500），并上移避开计时器区域。',
            '🔧 修复收起状态被覆盖：收起规则改为逐 class 组合枚举，避免特异性不足（110 对 120）被移动端展开规则压掉。',
            '🧭 退出对局时重置地图显隐状态，避免收起/放大状态残留到下一局。',
            '版本号递增至 v2.3.4（Android 端，与网页端 v2.2.3 功能对齐）。',
        ],
    },
    {
        version: 'v2.3.3',
    ...
    },
    {
        ...
        changes: [
            '🖼️ 街景查看升级：自动识别 360° 全景 / 平面照片，普通照片不再被拉伸成伪全景，改以平面模式平移缩放查看；支持手动切换 自动 / 360° / 平面 三种模式。',
            '🔄 拖动方向设置：街景右上角 ⚙ 可单独反转水平 / 垂直拖动方向，设置本地记忆。',
            '版本号递增至 v2.1.1。',
        ],
    },
    {
        ...
        changes: [
            '👤 账号入口优化：进入游戏先弹出账号面板，支持注册 / 登录 / 第三方登录，游客可一键跳过（不影响立即游玩）。',
            '🔒 邮箱隐私强化：邮箱仅作为账号标识，服务端以 HMAC 摘要加密存储，任何接口都不再回传明文邮箱；注册不再需要验证码。',
            '🚀 街景加载提速：CDN 直连超时（例如部分地区网络屏蔽 fbcdn）自动回退后端代理，单源 12s 超时不再长时间卡死。',
            '版本号递增至 v2.1.0。',
        ],
    },
    {
