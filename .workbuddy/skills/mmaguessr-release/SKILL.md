...
**1. `config.js` 不能整文件覆盖** —— App 端有网页端没有的 Capacitor 逻辑
（`APK_USE_BACKEND`、`isCapacitor`、`API_BASE` 三段式判断）。直接 `cp` 会把这段逻辑冲掉。
正确做法是**逐处定点修改**：只移植本次改动的片段，保留 App 端特有代码。

**2. 密钥/长字符串一律程序化复制，绝不手打**：

工作环境的工具输出会对长密钥**脱敏**（把密钥替换成短占位串显示）。
如果你肉眼读了输出、再手打到另一个文件，**写进去的是占位串而不是真密钥**。
必须写脚本从源文件读字面量、直接注入目标文件：

```js
// 从网页端提取含引号的字面量，不经过 stdout
const web = fs.readFileSync(WEB, 'utf8');
const ai = web.indexOf('apiKey:');
const q1 = web.indexOf("'", ai), q2 = web.indexOf("'", q1 + 1);
const keyLiteral = web.slice(q1, q2 + 1);        // 直接注入，不打印
```

**验证也必须用哈希而非肉眼**（打印出来的都是占位串，看着"不一致"其实是对的）：
