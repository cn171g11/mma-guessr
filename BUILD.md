...
> release 包已用 `android/app/mma-release.keystore` 正式签名；`bash build.sh release` 直接出签名包。
...
APK 已启用 **CapacitorHttp**（`capacitor.config.json` 的 `plugins.CapacitorHttp.enabled=true`）：WebView 内的 fetch 走原生 HTTP，**不受浏览器 CORS 限制**（原生请求不带 Origin，后端 cors.go 对无 Origin 请求直接放行）。因此 APK 无需后端修改 CORS 即可访问生产后端 `https://tuxun.edu-group.cn`（街景搜索/全景解析/账号等联网功能均可用；后端不可达时前端自动降级为纯本地单机模式，但街景无法加载）。
