...
async function main() {
  ...
  const valid = results.filter(r => r.result.valid).map(r => ({
    name: r.cand[0], lat: r.result.lat, lng: r.result.lng,
    region: r.cand[3], difficulty: r.cand[4], imageId: r.result.imageId
  }));
