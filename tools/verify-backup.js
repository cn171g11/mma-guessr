...
for (const tag of TAGS) {
    ...
    if (!expected) {
        errors.push('SHA256SUMS 中缺失条目');
    } else {
        const expHash = expected.split(' ')[0];
        const actHash = crypto.createHash('sha256').update(fs.readFileSync(zipPath)).digest('hex');
        if (expHash !== actHash)
            errors.push(`SHA256 不匹配: 期望 ${expHash.slice(0, 12)} 实际 ${actHash.slice(0, 12)}`);
    }

    // 2) 文件数一致性（zip 内 vs git 树）
    const gitFiles = sh(`git ls-tree -r --name-only ${tag}`)
        .split('\n')
        .map((s) => s.replace(/\r$/, ''))
        .filter(Boolean);
    ...
    const pyOut = execSync(`python -c ${JSON.stringify(pyScript)} ${JSON.stringify(zipPath)} ${JSON.stringify(tag)}`, {
        encoding: 'utf8',
    });
    const zipList = pyOut
        .split('\n')
    ...
    if (gitFiles.length !== zipList.length) {
        ...
        for (let i = 0; i < gs.length; i++) {
            if (gs[i] !== zs[i]) {
                errors.push(`文件列表不一致: ${gs[i]} vs ${zs[i]}`);
                break;
            }
        }
    }
